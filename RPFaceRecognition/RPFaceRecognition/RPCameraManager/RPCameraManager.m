...
#import "UIImage+RPResize.h"

NSString * const touchFocus = @"TouchFocus";
...
@interface RPCameraManager () <AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureMetadataOutputObjectsDelegate>
...
@implementation RPCameraManager
- (void)dealloc
{
...
- (void)switchCamera:(BOOL)isFrontCamera didFinishChanceBlock:(void (^)(void))block
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [self->_session removeInput:self->_inputDevice];
...
- (void)setFocusObserver:(BOOL)yes
{
    ...
    if (device && [device isFocusPointOfInterestSupported]) {
        if (yes) {
            [device addObserver:self forKeyPath:touchFocus options:NSKeyValueObservingOptionNew|NSKeyValueObservingOptionOld context:nil];
        }else{
            [device removeObserver:self forKeyPath:touchFocus context:nil];
        }
    }else{
        ShowAlert(@"你的设备没有照相机");
    }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:touchFocus]) {
    ...
    if (self.canFaceRecognition) {
        for(AVMetadataObject *metadataObject in metadataObjects) {
            if([metadataObject.type isEqualToString:AVMetadataObjectTypeFace]) {
                ...
                [self showFaceImageWithFrame:objec.bounds];
            }
        }
    }
}
